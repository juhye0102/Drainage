...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'Flow_Direction_dialog_base.ui'))

_layerPath=""
_util = util()
class Flow_DirectionDialog(QDialog, FORM_CLASS):
    ...
    def Click_Okbutton(self):
        ...
        arg = _util.GetTaudemArg(_layerPath, self.txtOutput.text(), _util.tauDEMCommand.FD, False,0)
        _util.MessageboxShowInfo("arg",arg)
        returnValue = _util.Execute(arg)
        if returnValue == 0:
            self.Addlayer_OutputFile(self.txtOutput.text())
            _util.MessageboxShowInfo("Flow Direction","processor complete")
            self.close()
    ...
    def __init__(self, parent=None):
        """Constructor."""
        super(Flow_DirectionDialog, self).__init__(parent)
        self.setupUi(self)
